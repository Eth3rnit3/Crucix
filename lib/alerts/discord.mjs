...
function getTierConfig() {
  return {
    FLASH:    { color: 0xFF0000, label: t('bot.alertTiers.flash', 'FLASH'),    cooldownMs: 5 * 60 * 1000,  maxPerHour: 6 },
    PRIORITY: { color: 0xFFAA00, label: t('bot.alertTiers.priority', 'PRIORITY'), cooldownMs: 30 * 60 * 1000, maxPerHour: 4 },
    ROUTINE:  { color: 0x3498DB, label: t('bot.alertTiers.routine', 'ROUTINE'),  cooldownMs: 60 * 60 * 1000, maxPerHour: 2 },
  };
}
...
function getSlashCommands() {
  return [
    { name: 'status',    description: t('discord.commands.status', 'System health, last sweep time, source status') },
    { name: 'sweep',     description: t('discord.commands.sweep', 'Trigger a manual sweep cycle') },
    { name: 'brief',     description: t('discord.commands.brief', 'Compact intelligence summary') },
    { name: 'portfolio', description: t('discord.commands.portfolio', 'Portfolio status (if Alpaca connected)') },
    { name: 'alerts',    description: t('discord.commands.alerts', 'Recent alert history') },
    { name: 'mute',      description: t('discord.commands.mute', 'Mute alerts (default 1h)'),
      options: [{ name: 'hours', description: t('discord.commands.muteHoursOption', 'Hours to mute (default: 1)'), type: 10, required: false }] },
    { name: 'unmute',    description: t('discord.commands.unmute', 'Resume alerts') },
  ];
}
...
export class DiscordAlerter {
  ...
  async _handleCommand(interaction) {
    ...
    if (name === 'mute') {
      ...
      const muteDesc = t('alerts.messages.mutedUntil', 'Alerts silenced for {hours}h — until {time} UTC.')
        .replace('{hours}', hours)
        .replace('{time}', new Date(this._muteUntil).toLocaleTimeString()) +
        '\n' + t('alerts.messages.useUnmuteToResume', 'Use /unmute to resume.');
      await interaction.reply({
        embeds: [this._embed(t('alerts.messages.alertsMuted', 'Alerts Muted'), muteDesc, 0x95A5A6)],
        ephemeral: true,
      });
      return;
    }
    ...
    if (name === 'unmute') {
      this._muteUntil = null;
      await interaction.reply({
        embeds: [this._embed(t('alerts.messages.alertsResumed', 'Alerts Resumed'), t('alerts.messages.willReceiveNext', 'You will receive the next signal evaluation.'), 0x2ECC71)],
        ephemeral: true,
      });
      return;
    }
    ...
    if (name === 'alerts') {
      ...
      if (recent.length === 0) {
        await interaction.reply({ content: t('bot.messages.noRecentAlerts', 'No recent alerts.'), ephemeral: true });
        return;
      }
      ...
      await interaction.reply({
        embeds: [this._embed(t('bot.messages.recentAlerts', 'Recent Alerts ({count})').replace('{count}', recent.length), lines.join('\n'), 0x3498DB)],
        ephemeral: true,
      });
      return;
    }
  ...
  async evaluateAndAlert(llmProvider, delta, memory) {
    ...
    const TIER_CONFIG = getTierConfig();
  ...
  _buildAlertEmbed(evaluation, delta, tier) {
    ...
    // Add fields
    const fields = [
      { name: t('alerts.fields.direction', 'Direction'), value: delta.summary.direction.toUpperCase(), inline: true },
      { name: t('alerts.fields.confidence', 'Confidence'), value: `${confidenceEmoji} ${evaluation.confidence || 'MEDIUM'}`, inline: true },
    ];

    if (evaluation.crossCorrelation) {
      fields.push({ name: t('alerts.fields.crossCorrelation', 'Cross-Correlation'), value: evaluation.crossCorrelation, inline: true });
    }

    if (evaluation.actionable && evaluation.actionable !== t('alerts.fields.monitor', 'Monitor')) {
      fields.push({ name: '💡 ' + t('alerts.fields.action', 'Action'), value: evaluation.actionable, inline: false });
    }
