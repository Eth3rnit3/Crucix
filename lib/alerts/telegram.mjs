...
function getTierConfig() {
  return {
    FLASH:    { emoji: '🔴', label: t('bot.alertTiers.flash', 'FLASH'),    cooldownMs: 5 * 60 * 1000,  maxPerHour: 6 },
    PRIORITY: { emoji: '🟡', label: t('bot.alertTiers.priority', 'PRIORITY'), cooldownMs: 30 * 60 * 1000, maxPerHour: 4 },
    ROUTINE:  { emoji: '🔵', label: t('bot.alertTiers.routine', 'ROUTINE'),  cooldownMs: 60 * 60 * 1000, maxPerHour: 2 },
  };
}
...
function getCommands() {
  return {
    '/status':    t('bot.commands.status', 'Get current system health, last sweep time, source status'),
    '/sweep':     t('bot.commands.sweep', 'Trigger a manual sweep cycle'),
    '/brief':     t('bot.commands.brief', 'Get a compact text summary of the latest intelligence'),
    '/portfolio': t('bot.commands.portfolio', 'Show current positions and P&L (if Alpaca connected)'),
    '/alerts':    t('bot.commands.alerts', 'Show recent alert history'),
    '/mute':      t('bot.commands.mute', 'Mute alerts for 1h (or /mute 2h, /mute 4h)'),
    '/unmute':    t('bot.commands.unmute', 'Resume alerts'),
    '/help':      t('bot.commands.help', 'Show available commands'),
  };
}
  ...
  async _handleMessage(msg) {
    ...
    if (command === '/help') {
      ...
      await this.sendMessage(
        `🤖 *CRUCIX BOT COMMANDS*\n\n${helpText}\n\n_${t('bot.messages.commandsTip', 'Tip: Commands are case-insensitive')}_`,
    ...
    if (command === '/mute') {
      ...
      const muteMsg = t('bot.messages.alertsMuted', '🔇 Alerts muted for {hours}h — until {time} UTC')
        .replace('{hours}', hours)
        .replace('{time}', new Date(this._muteUntil).toLocaleTimeString());
      await this.sendMessage(
        `${muteMsg}\n${t('bot.messages.useUnmute', 'Use /unmute to resume.')}`,
        { replyToMessageId: msg.message_id }
      );
      return;
    }
    ...
    if (command === '/unmute') {
      ...
      await this.sendMessage(
        t('bot.messages.alertsResumed', '🔔 Alerts resumed. You\'ll receive the next signal evaluation.'),
        { replyToMessageId: msg.message_id }
      );
      return;
    }
    ...
    if (command === '/alerts') {
      ...
      if (recent.length === 0) {
        await this.sendMessage(t('bot.messages.noRecentAlerts', 'No recent alerts.'), { replyToMessageId: msg.message_id });
        return;
      }
      ...
      await this.sendMessage(
        t('bot.messages.recentAlerts', '📋 Recent Alerts (last {count})').replace('{count}', recent.length) + `\n\n${lines.join('\n')}`,
        { replyToMessageId: msg.message_id }
      );
      return;
    }
